...
PlacementResult adaptivePlacement(const std::vector<WindowInfo> &windows,
  ...
  for (size_t i = 0; i < windowCount; ++i) {
    ...
    double sizeRatio =
        (windows[i].width * windows[i].height) / (screen.width * screen.height);
  ...
  for (size_t i = 0; i < windowCount; ++i) {
    ...
    result.tiles[i] = {screen.offsetX + x, screen.offsetY + y + verticalOffset,
                       windowWidths[i], windowHeights[i]};
  }
